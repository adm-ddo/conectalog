...
/** Avaliações que os motoboys deixaram sobre esse cliente ao encerrar o
 * turno — sentido inverso da AvaliacoesSection do motoboy (lá é o cliente
 * avaliando o motoboy). */
export default function AvaliacoesRecebidasSection({
  media,
  total,
  avaliacoes,
}: {
  media: number;
  total: number;
  avaliacoes: { id: number; nota: number; comentario: string | null; motoboyNome: string; data: string }[];
}) {
  return (
    <div className="rounded-2xl border border-stone-200 bg-white p-5 flex flex-col gap-3">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-semibold text-navy-900">Avaliações dos motoboys</h2>
          ...
          {avaliacoes.map((a) => (
            <li key={a.id} className="rounded-xl border border-stone-100 px-3 py-2">
              <div className="flex items-center justify-between gap-2">
                <span className="text-sm font-medium text-navy-900">{a.motoboyNome}</span>
